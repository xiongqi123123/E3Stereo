...
class hourglass(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Sequential(
            BasicConv(in_channels + 1, in_channels * 2, is_3d=True, bn=True, relu=True, kernel_size=3, padding=1,
                      stride=2, dilation=1),
            BasicConv(in_channels * 2, in_channels * 2, is_3d=True, bn=True, relu=True, kernel_size=3, padding=1,
                      stride=1, dilation=1))
    ...
    def forward(self, x, features, edge):
        # x: [B, 8, D, H, W]
        # edge: [B, 1, H, W]
        B, C, D, H, W = x.shape
        # 将 2D Edge 扩展为 3D Volume
        edge_3d = F.interpolate(edge, size=(H, W), mode='bilinear')
        edge_3d = edge_3d.unsqueeze(2).expand(B, 1, D, H, W)  # 复制 D 次

        # 拼接：变成 [B, 9, D, H, W]
        x_input = torch.cat([x, edge_3d], dim=1)
...
class IGEVStereo(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision, dtype=getattr(torch, self.args.precision_dtype, torch.float16)):
            ...
            geo_encoding_volume = self.cost_agg(gwc_volume, features_left, left_edge)
