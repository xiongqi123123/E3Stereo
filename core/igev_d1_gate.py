...
class IGEVStereo(nn.Module):
    def __init__(self, args):
        ...
        class GatedEdgeFusion(nn.Module):
            def __init__(self, in_dim):
                super().__init__()
                self.edge_proj = nn.Conv2d(1, in_dim, 1)  # 将 1 通道边缘映射到特征维度
                self.gate_conv = nn.Sequential(
                    nn.Conv2d(in_dim * 2, 1, kernel_size=1),  # 融合特征和边缘信息计算权重
                    nn.Sigmoid()  # 输出 0-1 的门控值
                )

            def forward(self, x, edge):
                # x: [B, C, H, W], edge: [B, 1, H, W]
                edge_feat = self.edge_proj(edge)
                # 计算门控权重：在边缘强的地方，gate 趋近于 1
                gate = self.gate_conv(torch.cat([x, edge_feat], dim=1))
                # 融合：原始特征 + 门控 * 边缘特征
                return x + gate * edge_feat

        # 在 IGEVStereo.__init__ 中替换：
        self.edge_fusion_blocks = nn.ModuleList([
            GatedEdgeFusion(context_dims[i])
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with torch.no_grad():
            left_edge_rcf = self.edge(image1)[-1]  # [B, 1, H, W], [0, 1], 原始 edge 供 loss 使用
            left_edge = left_edge_rcf * 5.0  # 缩放因子，使 edge 的有效范围变为 [0, 5]，供 fusion 使用
        ## =========== Edge ====================##
        ...
        with autocast(enabled=self.args.mixed_precision, dtype=getattr(torch, self.args.precision_dtype, torch.float16)):
            ...
            # 注意：建议使用原始概率图(0-1)，不要像学生那样乘 5.0，容易导致数值不稳定
            edge_resized = [F.interpolate(left_edge, size=inp.shape[2:], mode='bilinear') for inp in inp_list]
            inp_list = [self.edge_fusion_blocks[i](inp, edge_r) for i, (inp, edge_r) in
                        enumerate(zip(inp_list, edge_resized))]
            # === End Fusion ===#
