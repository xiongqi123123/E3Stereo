...
class IGEVStereo(nn.Module):
    def __init__(self, args):
        ...
        class EdgeSpatialAttention(nn.Module):
            ...
            def forward(self, x, edge):
                # 对边缘图进行简单卷积处理，适应特征尺度
                attn_map = self.sigmoid(self.conv(edge))
                # 核心：F_out = F_in * (1 + Edge_Attention)
                # 这样边缘处的特征会被放大，非边缘处保持原样
                return x * (1 + attn_map)

        self.edge_attn_blocks = nn.ModuleList([EdgeSpatialAttention() for _ in range(self.args.n_gru_layers)])
        # ================== End Edge Fusion ====================##
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision, dtype=getattr(torch, self.args.precision_dtype, torch.float16)):
            ...
            inp_list = [self.edge_attn_blocks[i](inp, edge_r) for i, (inp, edge_r) in
