...
def sequence_loss(disp_preds, disp_init_pred, disp_gt, valid, edge_map, loss_gamma=0.9, max_disp=192):
    # edge_map: [B, 1, H, W], 数值在 0-1 之间
    n_predictions = len(disp_preds)
    mag = torch.sum(disp_gt ** 2, dim=1).sqrt()
    # 过滤无效区域
    valid_mask = (valid >= 0.5) & (mag < max_disp)

    # 核心：构建权重图。边缘处权重为 1 + 2.0 * edge (即最高 3.0 倍权重)
    # 您可以调节 2.0 这个超参数
    pixel_weights = 1.0 + 2.0 * edge_map
    ...
    for i in range(n_predictions):
        ...
        # 计算每一次迭代的加权 Loss
        i_loss_val = (disp_preds[i] - disp_gt).abs()
